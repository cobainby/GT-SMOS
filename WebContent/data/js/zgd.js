...
//单次位移曲线
var myLineChartGap;
var zgdNames = new Array();
var zgdModel = new Array();
//累积变化量的最小值
var minAccum;
//累积变化量的最大值
var maxAccum;
...
//工程id
var projectUuid;
zgdNames = ['数据ID', '编号', '单次变化量(mm)', '累计变化量(mm)', '变化速率(mm/d)', '测量时间', '高程(m)'];
zgdModel = [{
...
$(function() {
    ...
    $("#dataExportButton").bind("click", function() {
        ...
        $.post("getProjectReportByProject", function(data, status) {
            ...
            if (reportDataObj != null) {
                var deviceName=reportDataObj.p1p86;
                var specification=reportDataObj.p1p87;
    ...
    $("#saveExportBtn").bind("click", function() {
        ...
        //导出报表
        window.open("/smosplat/zgdExport?pjName=" + pjName + "&pjAddress=" + pjAddress +"&deviceName="+deviceName+ "&specification=" + specification + "&bDateStr=" + bDate + "&eDateStr=" + eDate);
    });
    $("#zgdLineDivAccum").width($("#echarts").width());
    $("#zgdLineDivAccum").height($("#echarts").height());
    $("#zgdLineDivGap").width($("#echarts").width());
    $("#zgdLineDivGap").height($("#echarts").height());
});
function downUrl(fileName, path) {
    document.location.href = "/downloadSourceFile?fileName=" + fileName + "&path=" + path;
}
...
function init() {
    ...
    $.post("/smosplat/getLatestZGDatasByProject", function(data, status) {
        //先取出最新10条数据进行显示
        var dataObj = JSON.parse(data);
        projectUuid = dataObj.projectUuid;
        var surveyPoints = dataObj.surveyPoints;
        var zgdDatasObj = dataObj.levelData;
        if (dataObj.result == 0) {
            $('#loadingPage').remove();
            //保存所有时间值
            var times = new Array();
            ...
            for (var i = 0; i < surveyPoints.length; i++) {
                //根据编号拿到不同编号的数据
                var zgdDatas = zgdDatasObj[surveyPoints[i].code];
                //因为查询的数据是倒序的，所以取第0个就是最新的数据
                var lastzgdData = zgdDatas == undefined ? {} : zgdDatas[zgdDatas.length - 1];
                if (lastzgdData == undefined) {
                    lastzgdData = new Object;
                }
                lastzgdData["code"] = surveyPoints[i].code;
                lastzgdDatas.push(lastzgdData);
                //存累计变化量、不同编号点的时间
                if (zgdDatas != undefined) {
                    //保存时间点
                    for (var k = 0; k < zgdDatas.length; k++) {
                        if (isInArray(times, zgdDatas[k].surveyTime) == false) {
                            times.push(zgdDatas[k].surveyTime);
                        }
                    }
                }
            }
            $("#zgdDG").jqGrid({
                ...
                onSelectRow: function(rowId) {
                    //获取当前选择的行的ID
                    var selectedIds = $("#zgdDG").jqGrid("getGridParam", "selarrrow");
                    var rowDatas = [];
                    //获取当前选择行的数据
                    for (var l = 0; l < selectedIds.length; l++) {
                    	rowDatas.push($("#zgdDG").jqGrid("getRowData", selectedIds[l]));
                    }
                    //选中的点的时间集合
                    var selectTimes=[];
                    for(var k=0;k<rowDatas.length;k++){
                    	var zgdDatas = zgdDatasObj[rowDatas[k].code];
                    	if (zgdDatas != undefined) {
                            //保存时间点
                            for (var i = 0; i < zgdDatas.length; i++) {
                                if (isInArray(selectTimes, zgdDatas[i].surveyTime) == false) {
                                	selectTimes.push(zgdDatas[i].surveyTime);
                                }
                            }
                        }
                    }
                    getItemsData(selectTimes,rowDatas,zgdDatasObj);
                }
            });
            $("#zgdDG").setGridHeight($("#zgdGrid").height() - 80);
            $("#cb_zgdDG").click();
            $("#cb_zgdDG").bind("click", function() {
                if ($("#cb_zgdDG").is(':checked')) {
                	getItemsData(times,surveyPoints,zgdDatasObj);
                } else {
...
//取得选中的数据
function getItemsData(times,surveyPoints,zgdDatasObj){
    ...
    for (var i = 0; i < surveyPoints.length; i++) {
        //每一个监测点的最新的10条数据
        var zgdDataList = zgdDatasObj[surveyPoints[i].code];
        //单独取出每一条线的时间，用来和所有的时间点比较，得到当前测点哪个时间无返回数据
        var singleTime = new Array();
        var newZgdData=[];
        for (var o = 0; o < zgdDataList.length; o++) {
        	//去掉原数组中的重复上传的数据
        	if(isInArray(singleTime,zgdDataList[o].surveyTime)== false) {
        	　　singleTime.push(zgdDataList[o].surveyTime);
        	    newZgdData.push(zgdDataList[o]);
        	}
        }
        //无返回数据的时间给他赋空值
        ...
        //重新排序
        newZgdData.sort(function(a, b) {
            return a.surveyTime - b.surveyTime;
        });
        //取最新的10条
        if (newZgdData.length > 10) {
            newZgdData = newZgdData.slice(-10);
        }
        var serieDataForAccum = new Array();
        var serieDataForGap = new Array();
        if (newZgdData != undefined) {
            for (var j = 0; j < newZgdData.length; j++) {
                //判断当前测点是否有这个时间点的数据,没有则默认为空
                var value = newZgdData[j].accumHOffset;
                serieDataForAccum.push([newZgdData[j].surveyTime + "", value]);
                value = newZgdData[j].gapHOffset;
                serieDataForGap.push([newZgdData[j].surveyTime + "", value]);
            }
        }
    ...
    zgdCodes = new Array();
    for (var i = 0; i < surveyPoints.length; i++) {
        zgdCodes.push(surveyPoints[i].code);
    }
...
function initSocket(projectUuid) {
    ...
    stompClient.connect({}, function(frame) {
        var subscribeUrl = "/collect/projectZGDCollect/" + projectUuid;
        stompClient.subscribe(subscribeUrl, function(message) {
            ...
            //得到datagrid的数据源
            var dgData = $("#zgdDG").jqGrid("getRowData");
            ...
            //dqgrid重新加载数据
            $("#zgdDG").jqGrid('setGridParam', { // 重新加载数据
                datatype: "local",
                styleUI: 'Bootstrap',
                data: dgData,
//                colNames: zgdNames,
...
function getOption(times, seriesDataForGapLine, seriesDataForAccumLine) {
    ...
    //初始化折线图
    myLineChartAccum = echarts.init(document.getElementById('zgdLineDivAccum'), 'macarons');
    ...
    myLineChartGap = echarts.init(document.getElementById('zgdLineDivGap'), 'macarons');
